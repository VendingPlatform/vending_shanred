...
<body>
			...
			<div class="panel-body">
				<table class="table" align="center">
					...
					<c:forEach items="${channelGroups}" var="g">
						<tr>
							<td><a href="<c:url value='/channel/getChannelsByGroup/${g.channelGroupId}'/>">${g.channelGroupName}</a></td>
							...
							<td>${g.operateDate}</td>
							<td>
								<a onclick="getAllMachienChannelsNotAssign('<%=basePath %>',${user.firmInfo.firmId},'${g.channelGroupId}' )" 
								   data-toggle="modal" data-target="#addChannelsToGroup">
								<span class="glyphicon glyphicon-plus" title="添加货道"></span></a>
							</td>
						</tr>
					</c:forEach>
				</table>
	...
	<div class="modal fade" id="addChannelsToGroup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">添加货道</h4>
				</div>
				<div class="modal-body" style="width: 80%; padding-left: 30px">
					<form id="addChannelsToGroupForm">
						<input type="hidden" name="channelGroupId" id="channelGroupId">
						<input type="hidden" name="userId" value="${user.userId}">
						<div class="checkbox" id="channelsNotAssign"></div>
					</form>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" class="btn btn-primary" onclick="addChannelsToGroup('<%=basePath %>')" data-dismiss="modal">保存</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="modal fade" id="addChannelGroup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" class="btn btn-primary" onclick="addChannelGroup('<%=basePath%>')" data-dismiss="modal">保存</button>
				</div>
			</div>
		</div>
	</div>
